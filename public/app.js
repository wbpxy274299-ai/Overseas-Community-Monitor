...
function applyRolePermissions() {
  ...
  console.log('找到', tabs.length, '个标签按钮');
  
  // 新的权限结构：
  // 1. 发布模块（所有用户）：提交发送、任务列表
  // 2. 舆情板块（仅管理员）
  // 3. 周报板块（仅管理员）
  // 4. 权限管理板块（仅管理员）
  
  if (!isAdmin) {
    console.log('⚠️  普通用户模式 - 只显示发布模块');
    
    // 普通用户：显示前两个标签（发布模块），隐藏高级功能入口
    tabs.forEach((tab, index) => {
      if (index < 2) {
        tab.style.display = 'inline-block';
        console.log('  显示标签:', tab.textContent);
      } else {
        tab.style.display = 'none';
        console.log('  隐藏标签:', tab.textContent);
      }
    });
    ...
    if (identityInfo) {
      // 隐藏舆情监控、周报管理和权限管理按钮
      const sentimentBtn = identityInfo.querySelector('a[href="/sentiment"]');
      const reportsBtn = identityInfo.querySelector('a[href="/reports"]');
      const adminBtn = identityInfo.querySelector('a[href="/admin"]');
    ...
    const identityInfo = document.querySelector('.identity-info');
    if (identityInfo) {
      const buttons = identityInfo.querySelectorAll('a, button');
      buttons.forEach(btn => {
        btn.style.display = 'inline-block';
      });
      console.log('  显示所有身份栏按钮');
    }
  }
}
...
async function doAction(taskId, action, msgDivId) {
  // 获取任务信息用于确认提示
  const task = db_getTask(taskId); // 从本地缓存或重新获取
