...
async function viewReport(id, opts) {
  ...
  try {
    ...
    if (data.ok) {
      ...
      showReportInReader(renderMarkdown(r.content), reportArea);
...
async function generateInsights() {
  ...
  try {
    ...
    if (data.ok) {
      ...
      // 渲染报告
      showReportInReader(renderMarkdown(data.report), reportArea);
      openRpt('rptInsight');
...
// ===== 报告阅读器（V45 成品） =====
function showReportInReader(htmlStr, reportArea) {
  var body = document.getElementById('rptBody');
  if (body) body.innerHTML = htmlStr;
  if (reportArea) reportArea.innerHTML = '<div class="panel-cut" style="padding:28px 28px 26px;text-align:center">'
